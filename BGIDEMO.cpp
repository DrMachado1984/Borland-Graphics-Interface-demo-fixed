/*
   ...
   From the command line, use:
    ...
    Remastered by Dr. Machado ft. deepSeek, 2026
...
void RandomBars(void)
{
  ...
  while( !kbhit() ){			/* Until user enters a key...	*/
    color = rand()%( MaxColors-1 )+1;
    setcolor( color );
    setfillstyle( rand()%(11)+1, color );
    bar3d( rand()%( getmaxx() ), rand()%( getmaxy() ),
	   rand()%( getmaxx() ), rand()%( getmaxy() ), 0, OFF);
  }
...
void ArcDemo(void)
{
  ...
  while( !kbhit() ){			/* Repeat until a key is hit	*/
    setcolor( rand()%( MaxColors - 1 ) + 1 );	/* Randomly select a color	*/
    eangle = rand()%( 358 ) + 1; 	/* Select an end angle		*/
    arc( rand()%(MaxX), rand()%(MaxY), rand()%(eangle), eangle, mradius );
...
void CircleDemo(void)
{
  ...
  while( !kbhit() ){			/* Repeat until a key is hit	*/
    setcolor( rand()%( MaxColors - 1 ) + 1 );	/* Randomly select a color	*/
    circle( rand()%(MaxX), rand()%(MaxY), rand()%(mradius) );
  }					/* End of WHILE not KBHIT	*/
